...
<body>
  <main class="card">
    ...
    <p class="subtitle">按方向鍵或點按螢幕上的按鈕控制車子。放開按鍵後會自動送出停止指令。</p>
    ...
    <p class="hint">
      鍵盤支援：<code>ArrowUp</code> 前進、<code>ArrowDown</code> 後退、<code>ArrowLeft</code> 左轉、<code>ArrowRight</code> 右轉、<code>Space</code> 停止。
    </p>
  </main>
  ...
  <script>
    ...
    buttons.forEach((button) => {
      ...
      button.addEventListener("mousedown", () => press(command));
      button.addEventListener("mouseup", () => release(command));
      button.addEventListener("mouseleave", () => release(command));
      button.addEventListener("touchstart", (event) => {
        event.preventDefault();
        press(command);
      }, { passive: false });
      button.addEventListener("touchend", () => release(command));
      button.addEventListener("touchcancel", () => release(command));
    ...
    const ip = urlParams.get("ip") || "192.168.4.1";
