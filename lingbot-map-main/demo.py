...
def _get_world_points(pred_dict):
    """Return world-space points, deriving them from depth if needed."""
    world_points = pred_dict.get("world_points")
    if world_points is not None:
        return world_points

    depth = pred_dict.get("depth")
    extrinsic = pred_dict.get("extrinsic")
    intrinsic = pred_dict.get("intrinsic")
    if depth is None or extrinsic is None or intrinsic is None:
        raise ValueError("PLY export requires world_points, or depth with camera intrinsics/extrinsics.")
...
def _filter_points_for_export(points, colors, confidence, conf_threshold, downsample_factor):
    """Flatten, filter, and downsample one frame of point cloud data."""
    points = points.reshape(-1, 3)
    colors = colors.reshape(-1, 3)
    confidence = confidence.reshape(-1) if confidence is not None else np.ones(len(points), dtype=np.float32)
    ...
    if valid.any():
        ...
        return points[:0], colors[:0], confidence[:0]
...
def export_predictions_to_ply(
    ...
    image_folder=None,
    sky_mask_dir=None,
    sky_mask_visualization_dir=None,
):
    """Export demo predictions to a binary little-endian PLY point cloud."""
    if downsample_factor < 1:
        raise ValueError("--downsample_factor must be at least 1 for PLY export.")

    world_points = _get_world_points(pred_dict)
    images = pred_dict.get("images")
    if images is None:
        raise ValueError("PLY export requires images for point colors.")

    confidence = pred_dict.get("world_points_conf", pred_dict.get("depth_conf"))
    ...
    if images.ndim != 4:
        raise ValueError(f"Expected images with 4 dimensions, got shape {images.shape}.")
    if images.shape[1] == 3:
    ...
    if world_points.shape[:3] != colors.shape[:3]:
        raise ValueError(
            f"Point/image shape mismatch: points {world_points.shape}, images {images.shape}."
        )
    ...
    for frame_idx in range(world_points.shape[0]):
        pts, _, _ = _filter_points_for_export(
            world_points[frame_idx],
            colors[frame_idx],
    ...
    if filtered_count == 0:
        raise ValueError(
            "No points passed the PLY export filters. Try lowering --conf_threshold "
            "or --downsample_factor."
        )

    header = (
        "ply\n"
        "format binary_little_endian 1.0\n"
        "comment generated by LingBot-MAP demo.py\n"
        f"element vertex {filtered_count}\n"
        "property float x\n"
        "property float y\n"
        "property float z\n"
        "property uchar red\n"
    ...
    with open(output_path, "wb") as handle:
        ...
        for frame_idx in range(world_points.shape[0]):
            pts, cols, conf = _filter_points_for_export(
                world_points[frame_idx],
                colors[frame_idx],
                confidence[frame_idx],
                conf_threshold,
            ...
            if len(pts) == 0:
                continue
            rows = np.empty(len(pts), dtype=dtype)
            rows["x"] = pts[:, 0].astype(np.float32, copy=False)
            rows["y"] = pts[:, 1].astype(np.float32, copy=False)
            rows["z"] = pts[:, 2].astype(np.float32, copy=False)
            rows["red"] = cols[:, 0]
            rows["green"] = cols[:, 1]
            rows["blue"] = cols[:, 2]
            rows["confidence"] = conf.astype(np.float32, copy=False)
            rows.tofile(handle)

    print(
        f"PLY exported to {output_path} ({filtered_count:,} points, "
        f"conf>{conf_threshold}, downsample={downsample_factor})"
    )
...
def main():
    ...
    parser.add_argument("--output_ply", type=str, default=None,
                        help="Export the reconstructed point cloud to a binary PLY file before launching the viewer")
